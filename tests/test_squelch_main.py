...
# * By default, we print an error to stderr and quit
# * In DEBUG mode, we show the exception and stack trace
(['main', '-c', '/non-existent.json'], SystemExit),
(['main', '-c', '/non-existent.json', '-u', 'd://u:p@h/db', '-v'], SystemExit),
(['main', '-c', '/non-existent.json', '-v'], SystemExit),
(['main', '-c', '/non-existent.json', '-vv'], FileNotFoundError),
(['main', '-c', '/non-existent.json', '-u', 'd://u:p@h/db', '-vv'], FileNotFoundError),
(['main', '-c', '/non-existent.json', '-vv'], FileNotFoundError),
])
def test_consolidate_conf_error(unconfigured_squelch, argv, expected):
    f = unconfigured_squelch
    sys.argv = argv
    args = m.parse_cmdln()
